 ...
 */
	...
	function renderShows(shows) {
		...
		shows.forEach(function (show) {
			var article = template.replace(':name:', show.name).replace(':img:', show.image.medium).replace(':img alt:', show.name + ' Logo').replace(':summary:', show.summary);
			...
			$showsContainer.append($article.fadeIn('slow'));
		});
	}
	...
	if (!localStorage.shows) {
		$.ajax('http://api.tvmaze.com/shows').then(function (shows) {
			$showsContainer.find('.loader').remove();
...
//slow,fast,etc o numeros en ms
//Parametros: efecto(estils) y tiempo.
//Parametros: efecto(estils) y tiempo.
